...
#Read in the files
dat<-read.csv("parsnp_clustered_snv_counts.tsv",header=T,sep="\t")
...
#Remove sets of isolates that are not in the same cluster
dat1<-dat1[dat1$value !=1e+06,]
...
my_title = paste("SNP distribution inter and intra patient (pre-clustered) \n \n inter-peak=",x_coord_other," \n intra-peak=",x_coord_serial)
...
ggsave(filename="test2.pdf", width = 10, height =12, plot=p)
